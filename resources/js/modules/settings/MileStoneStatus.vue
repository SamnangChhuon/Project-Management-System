<template>
    <div class="content-wrapper">
        ...
        <content-header page-title="Milestone Statuses" page-name="Milestone Statuses"></content-header>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Milestone Status Table</h3>
...
<script>
    export default {
        ...
        methods:{
            getResults(page = 1) {
                axios.get('/api/milestones-statuses?page=' + page)
                    .then(response => {
            ...
            updateData() {
                ...
                this.form.put('/api/milestones-statuses/' + this.form.id)
                .then(() => {
                    // if success
                    $('#addNew').modal('hide');
                    Swal.fire(
            ...
            deleteStatus(id) {
                Swal.fire({
                    ...
                    if (result.value) {
                        this.form.delete('/api/milestones-statuses/' + id).then(() => {
                            Swal.fire(
                                'Deleted!',
            ...
            loadData() {
                ...
                //     }
                    axios.get("/api/milestones-statuses").then(({ data }) => (this.statuses = data));
            },
            createData() {
                this.$Progress.start();
                this.form.post('/api/milestones-statuses')
                .then(() => {
                    // If Insert Success
                    Fire.$emit('AfterCreate'); // Register new event "AfterCreate"
                    $('#addNew').modal('hide');
