...
<script>
    export default {
        ...
        methods:{
            getResults(page = 1) {
                axios.get('api/milestones/status?page=' + page)
                    .then(response => {
            ...
            updateData() {
                ...
                this.form.put('api/milestones/status/' + this.form.id)
                .then(() => {
                    // if success
                    $('#addNew').modal('hide');
                    Swal.fire(
            ...
            deleteStatus(id) {
                Swal.fire({
                    ...
                    if (result.value) {
                        this.form.delete('api/milestones/status/' + id).then(() => {
                            Swal.fire(
                                'Deleted!',
            ...
            loadData() {
                ...
                //     }
                    axios.get("api/milestones/status").then(({ data }) => (this.statuses = data));
            },
            createData() {
                this.$Progress.start();
                this.form.post('api/milestones/status')
                .then(() => {
                    // If Insert Success
                    Fire.$emit('AfterCreate'); // Register new event "AfterCreate"
                    $('#addNew').modal('hide');
